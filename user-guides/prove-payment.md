...
Haven, however, is private: that information is not available publicly on the blockchain. The steps are therefore a bit
more involved.
...
- the transaction's key, which is new with Haven and other CryptoNote currencies

When Alice made the transaction, a one time key was automatically generated just for this transaction.

#### CLI

Alice can query it thus in haven-wallet-cli (new name for the old simplewallet):
...
Note that this will only work if haven-wallet-cli is set to save transaction keys. To double check:
...
Alice can open her haven-wallet-gui and go to the history page to see her transaction detail:
...
Charlie types in haven-wallet-cli:

> check_tx_key TXID TXKEY ADDRESS

The information supplied by Alice plugs neatly instead of the placeholders. haven-wallet-cli will use the transaction
...
Charlie will open his haven-wallet-gui and go to the Advanced > Prove/Check page to fill the Check section with the informations provided by Alice:
