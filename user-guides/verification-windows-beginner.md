...
Verification of the Haven binary files should be done prior to extracting, installing, or using the Haven software. This is the only way to ensure that you are using the official Haven binary. If you receive a fake binary (eg. phishing, MITM, etc.), following this guide will protect you from being tricked into using it.
...
### [2. Import Signing Key](#2-haven-signing-key)
...
This section will cover downloading the Haven signing key, verifying that the key is correct, and then importing the key to your keyring. The hash file that will be used to verify your binary is cryptographically signed with the Haven signing key. In order to check the validity of this file you must have the public version of the signing key.

### 2.1. Download Signing Key

In a web browser, go to [binaryFate's GPG key](https://raw.githubusercontent.com/haven-project/haven/master/utils/gpg_keys/binaryfate.asc), which he uses for signing the Haven binaries. Right click on the page, choose `Save Page As`.
...
In a web browser, go to the [gethaven.org hash page]({{ site.baseurl_root }}/downloads/hashes.txt). Right click the page, select `Save Page As`.

![hashes right click](png/verify_binary_windows_beginner/verify-win_hashes-gethaven-rightclick.png)

Leave the default location, click `Save`.

![hashes save file](png/verify_binary_windows_beginner/verify-win_hashes-gethaven-savename.png)
...
This section will cover downloading the Haven binary and verifying its authenticity.

### 4.1. Download Binary

In a web browser, go to the [gethaven.org downloads page]({{ site.baseurl_root }}/downloads/#windows). Select the correct binary for your system.

![binary gethaven](png/verify_binary_windows_beginner/verify-win_binary-gethaven-windowsfiles.png)

Leave `Save File` selected, click `OK`.

![binary gethaven save](png/verify_binary_windows_beginner/verify-win_binary-gethaven-save-file.png)

Leave the default location, click `Save`.

![binary gethaven save location](png/verify_binary_windows_beginner/verify-win_binary-gethaven-save-location.png)
...
Calculate the hash of the Haven binary with the command: `certUtil -hashfile haven-gui-win-x64-v0.11.1.0.zip SHA256` (if you downloaded a command-line only version, replace `haven-gui-win-x64-v0.11.1.0.zip` accordingly).
...
If your hash **DOES** match then you are finished with verification! You can be sure the Haven files you have are authentic. You may extract and install/use the files normally.

If your hash **DOES NOT** match **DO NOT CONTINUE.** Instead delete the Haven binary from the `Downloads` directory and go back to [section 4.1](#41-download-binary).
