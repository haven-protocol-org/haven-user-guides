...
`haven-wallet-cli` is the wallet software that ships with the Haven tree. It is a console program,
and manages an account. While a bitcoin wallet manages both an account and the blockchain,
Haven separates these: `havend` handles the blockchain, and `haven-wallet-cli` handles the account.

This guide will show how to perform various operations from the `haven-wallet-cli` UI. The guide assumes you are using the most recent version of Haven and have already created an account according to the other guides.


## Checking your balance

Since the blockchain handling and the wallet are separate programs, many uses of `haven-wallet-cli`
need to work with the @daemon. This includes looking for incoming transactions to your address.
Once you are running both `haven-wallet-cli` and `havend`, enter `balance`.
...
Replace `ADDRESS` with the address you want to send to, `AMOUNT` with how many haven you want to send,
...
If you have your own Haven address, you just need to give your standard address to someone.

You can find out your address with:

    address

Since Haven is anonymous, you won't see the origin address the funds you receive came from. If you
...
mistake. Haven is private, so you can't just point to your transaction in the blockchain,
as you can't tell who sent it, and who received it. However, by supplying the per-transaction
private key to a party, that party can tell whether that transaction sent haven to that
...
If you are the third party (that is, someone wants to prove to you that they sent haven
...
address which were supplied to you, respectively. haven-wallet-cli will check that transaction
and let you know how much haven this transaction paid to the given address.
