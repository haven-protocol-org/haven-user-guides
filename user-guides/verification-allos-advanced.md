...
Verification of the Haven binary files should be done prior to extracting, installing, or using the Haven software. This is the only way to ensure that you are using the official Haven software. If you receive a fake Haven binary (eg. phishing, MITM, etc.), following this guide will protect you from being tricked into using it.

To protect the integrity of the binaries the Haven team provides a cryptographically signed list of all the [SHA256](https://en.wikipedia.org/wiki/SHA-2) hashes. If your downloaded binary has been tampered with it will be produce a [different hash](https://en.wikipedia.org/wiki/File_verification) than the one in the file.
...
### [4. Download & Verify Binary](#4-download-and-verify-binary)
  + [4.1. Get Haven Binary](#41-get-haven-binary)
...
This section will cover getting the Haven signing key, making sure it is correct, and importing the key to GnuPG.

### 2.1. Get Signing Key

On Windows or Mac, go to [binaryFate's GPG key](https://raw.githubusercontent.com/haven-project/haven/master/utils/gpg_keys/binaryfate.asc), which he uses to sign the Haven binaries, and save the page as `binaryfate.asc` to your home directory.

On Linux, you can download binaryFate's signing key by issuing the following command:

```
wget -O binaryfate.asc https://raw.githubusercontent.com/haven-project/haven/master/utils/gpg_keys/binaryfate.asc
```
...
uid                           binaryFate <binaryfate@gethaven.org>
```
...
gpg: key F0AF4D462A0BDF92: public key "binaryFate <binaryfate@gethaven.org>" imported
...
```
gpg: key F0AF4D462A0BDF92: "binaryFate <binaryfate@gethaven.org>" not changed
gpg: Total number processed: 1
gpg:              unchanged: 1
```
...
On Windows or Mac, go to the [hashes file on gethaven.org]({{ site.baseurl_root }}/downloads/hashes.txt) and save the page as `hashes.txt` to your home directory.
...
gpg: Good signature from "binaryFate <binaryfate@gethaven.org>" [unknown]
...
This section will cover downloading the Haven binary for your operating system, getting the `SHA256` hash of your download, and verifying that it is correct.

### 4.1. Get Haven binary

On Windows or Mac, go to [gethaven.org]({{ site.baseurl_root }}/downloads/) and download the correct file for your operating system. Save the file to your home directory. **Do not extract the files yet.**

On Linux, you can download the command line tools by issuing the following command:

```
wget -O haven-linux-x64-v0.15.0.1.tar.bz2 https://downloads.gethaven.org/cli/linux64
```

### 4.2. Binary Verification on Linux or Mac

The steps for both Linux and Mac are the same. From a terminal, get the `SHA256` hash of your downloaded Haven binary. As an example this guide will use the `Linux, 64bit` GUI binary. Substitute `haven-gui-linux-x64-v0.15.0.1.tar.bz2` with the name of the binary that you downloaded in [section 4.1](#41-get-haven-binary).

```
shasum -a 256 haven-linux-x64-v0.15.0.1.tar.bz2
```
...
```
8d61f992a7e2dbc3d753470b4928b5bb9134ea14cf6f2973ba11d1600c0ce9ad  haven-linux-x64-v0.15.0.1.tar.bz2
```

If your hash **DOES** match, then you are finished with the guide! You can extract the files and install.

If your hash **DOES NOT** match, **DO NOT CONTINUE.** Instead delete the binary you downloaded and go back to [section 4.1](#41-get-haven-binary).

### 4.3. Binary Verification on Windows

From a terminal, get the `SHA256` hash of your downloaded Haven binary. As an example this guide will use the `Windows, 64bit` GUI binary. Substitute `haven-gui-win-x64-v0.15.0.1.zip` with the name of the binary that you downloaded in [section 4.1](#41-get-haven-binary).

```
certUtil -hashfile haven-gui-win-x64-v0.15.0.1.zip SHA256
```
The output will look like this, but will be different for each binary file. Your `SHA256` hash should match the one listed in the `hashes.txt` file for your binary file.

```
SHA256 hash of file haven-gui-win-x64-v0.12.0.0.zip:
